...
namespace FirstServiceLibrary
{
    public class FirstSimpleWCF : IFirstWCF
    {
        ...
        public Data MySecondMethod(Data inputText)
        {
            Console.WriteLine("Message Received : {0}", inputText.Message);
            Console.WriteLine(OperationContext.Current.RequestContext.RequestMessage.ToString());
            return new Data { Message = string.Format("Message from server {0}", inputText.Message.Reverse()) };
        }
    }
}
